...
async fn main() -> Result<()> {
    ...
    // listen for lines that we have typed to be send from `stdin`
...
fn input_loop(line_tx: tokio::sync::mpsc::Sender<String>) -> Result<()> {
    ...
    loop {
        // loop through reading the buffer
        stdin.read_line(&mut buffer)?;
        // and thne sending over the channel
        line_tx.blocking_send(buffer.clone())?;
        // clear the buffer after we've send the content
        buffer.clear();
    }
}
